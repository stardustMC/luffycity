      ...
      <div class="cart-header">
        <div class="cart-header-warp">
          <div class="cart-title left">
            ...
            <div class="left">
              共<span>{{cart.cart_list.length}}</span>门，已选择<span>{{cart.cart_list.filter(item=>item.selected).length}}</span>门
            </div>
          </div>
          <div class="right">
            <div class="">
...
const get_cart_list = ()=>{
  ...
  cart.get_cart_list(token).then(response => {
    cart.cart_list = response.data.cart;
...
watch(()=>state.checked, ()=>{
  if(state.checked || cart.cart_list.every(course=>course.selected)){
    cart.cart_list.forEach(course => {
      course.selected = state.checked;
    })
  }
})
</script>
